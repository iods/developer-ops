...
if [ ! -f "${RELEASEFOLDER}/vendor/bin/install.sh" ] ; then echo "Could not find installer" ; exit 1; fi
${RELEASEFOLDER}/vendor/bin/install.sh -e "${ENVIRONMENT}" || { echo "Installing package failed"; exit 1; }
